...
export default function TherapyServices() {
  return (
    <>
      <section className="bg-white md:px-12 lg:px-28 px-6 mb-20">
        {/* Breadcrumb */}
        <p className="text-sm text-gray-400 py-14">
          <Link href="/">Home</Link> <span className="text-black">› Services</span>
        </p>

        <h2 className="text-3xl md:text-5xl font-semibold mb-10 text-black">Therapy Services</h2>
        ...
        <div className="grid gap-14 md:grid-cols-2 lg:grid-cols-3 grid-cols-1 justify-center">
          {services.map((service, idx) => (
            <div key={idx} className="transition flex flex-col h-full">
              <div className="lg:w-[415px] lg:h-[415px] w-full h-full justify-center rounded-2xl overflow-hidden mb-4">
                <Image
                  src={service.image}
                  alt={service.title}
                  width={415}
              ...
              <h3 className="font-semibold text-xl mb-2 text-black">{service.title}</h3>
      ...
      <div className="bg-[#FFFCF5] py-18 px-6 md:px-28 flex flex-col md:flex-row items-center justify-between">
